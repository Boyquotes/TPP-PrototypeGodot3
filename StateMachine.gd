...
func _unhandled_input(event):
	# delegate the _unhandled_input handling to the current staet
...
func transition_to(state_path: NodePath):
	# if there's no node at the specific path, don't do anything
	...
	# exit the current sate, set the crrent to the new state and enter it, and emit transitioned signal
	_state.exit()
	_state = new_state
	new_state.enter()
