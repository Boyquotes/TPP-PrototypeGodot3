...
func _process(delta):
	# get the camera's current horizontal and vertical rotation angles and assign them to local field
...
func _physics_process(delta):
	# lerp the horizontal and vertical gimbals' rotations towards the corresponding rotation angles
	# uses 'lerp' instead of 'lerp_angle' because the latter tries to determine the rotation 
	# direction is based on the current and the target values, which would cause the camera to twitch around
	# the current value and not reach thee target value
